...
To preserve the same estimator as the canonical cohort-specific top-q
comparison, each replicate recomputes:

    B_early* = round(q * N_early*)
    B_all*   = round(q * N_all*) = round(q * N_all)

because a full-size bootstrap keeps N_all fixed while N_early* varies.
...
It also reproduces the canonical cohort-specific top-q early->all bootstrap
replicates exactly (C00, C11, selected_n, TP, capture, and total contrast)
when the canonical run used the same requested number of bootstrap replicates.
...
SCRIPT_VERSION = "2026-08-24-v3-standalone-clearml-shapley"
...
DEFAULT_TASK_NAME = "TAE 2026 Shapley: four-cell decision-aware decomposition"
...
DEFAULT_CONFIG: dict[str, Any] = {
    "analysis": {
        "name": "four_cell_symmetric_shapley",
    ...
    "bootstrap": {
        ...
        "full_size": True,
        "paired": True,
        "recompute_early_budget_each_replicate": True,
        "progress_every": 25,
    },
    "output": {
        "directory": "results/tae_2026_shapley",
        "overwrite": False,
...
def canonical_bootstrap_lookup(
    ...
    *,
    ...
    if len(subset) != expected_rows:
        raise AssertionError(
            f"{target}: expected {expected_rows} canonical top-q bootstrap rows "
            f"for first {replicates} replicates, got {len(subset)}"
        )
    ...
    for _, row in subset.iterrows():
        ...
        if key in lookup:
            raise AssertionError(
                f"Duplicate canonical bootstrap key {target}/{key}"
            )
        lookup[key] = row
...
def compare_bootstrap_to_canonical(
    *,
    target: str,
    q: float,
    replicate: int,
...
def bootstrap_four_cell(
    *,
    ...
    canonical_lookup = canonical_bootstrap_lookup(
        canonical_bootstrap,
        target=target,
    ...
    for replicate in range(replicates):
        # Exactly the same nonparametric full-size bootstrap construction
        # as the canonical script.
        ...
        # candidate set, then query both budget levels.
        ...
        for q in FRACTIONS:
            # Recompute the q-derived early budget inside each bootstrap
            # replicate, exactly as canonical Protocol A did.
            b_early = max(
                1, int(round(float(q) * early_sample_n))
            )

            # Full-size bootstrap has N_all* == N_all == n.
            b_all = max(
                1, int(round(float(q) * n))
            )

            if b_all > early_sample_n:
                raise RuntimeError(
                    f"{target}/q={q}/rep={replicate}: "
                    f"B_all={b_all:,} > resampled early N={early_sample_n:,}"
                )
            ...
            # Strongest reproducibility invariant:
            # C00 and C11 reproduce the original canonical top-q bootstrap.
            canonical = canonical_lookup[
                (float(q), int(replicate))
            ]
            compare_bootstrap_to_canonical(
                target=target,
                q=q,
                replicate=replicate,
                b_early=b_early,
...
def validate_output_counts(
    *,
    ...
    checks = {
        ...
        # Every bootstrap replicate reaches this output only after exact
        # replicate-level comparison with canonical top-q rows.
        "canonical_topq_bootstrap_reproduced": True,
...
def main() -> int:
    ...
    config = task.connect_configuration(
        ...
        name="shapley_config",
        description=(
            "Locked post-canonical four-cell symmetric Shapley configuration"
        ),
    )
    ...
    manifest = {
        ...
        "bootstrap": {
            ...
            "early_budget_recomputed_from_resampled_early_n": True,
            "all_budget_fixed_by_full_size_bootstrap_n": True,
        },
        "outputs": {
    ...
    print("All point/bootstrap canonical cross-checks: PASS")
